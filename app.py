...
def main():
    ...
    if bolag:
        ...
        # Beräkna mål och undervärdering för varje rad
        resultats = []
        ...
        # Filtrera på undervärdering över 0% (alltså över värderade inkluderas ej)
        df_display = df_display[
            (df_display["undervardering_genomsnitt_ars"] > 0) |
            (df_display["undervardering_genomsnitt_nastaar"] > 0)
        ].copy()

        # Sortera efter mest undervärderad först (högst undervärdering)
        ...
        st.subheader("Bolag sorterade efter mest undervärderad (undervärdering > 0%)")

        # Dropdown för att välja bolag att visa
        valt_bolag = st.selectbox("Välj bolag att visa", df_display["namn"].tolist())

        # Visa valda bolagets nyckeltal
        if valt_bolag:
        ...
        if not df.empty:
            ...
            st.info("Inga bolag sparade ännu att ta bort.")
